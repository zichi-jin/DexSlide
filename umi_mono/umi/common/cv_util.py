...
def parse_fisheye_intrinsics(json_data: dict) -> Dict[str, np.ndarray]:
    ...
    assert json_data['intrinsic_type'] == 'StereoI'
...
def parse_charuco_config(aruco_config_dict: dict):
    """Parses YAML file for ChArUco board properties."""
...
def detect_localize_charuco_tags(frame, camera_matrix, board, dictionary):
    """
    Detects a ChArUco board in a frame and estimates its pose.
    ...
    if ids is not None and len(ids) > 0:
        ...
        if retval and charuco_corners is not None and charuco_ids is not None and len(charuco_corners) > 3:
            ...
            if pose_found:
                ...
                print("No ChArUco board pose could be estimated.")
                return None
...
def detect_localize_aruco_tags(
    ...
    for this_id, this_corners in zip(ids, corners):
        ...
        # if D is all zero, skip undistortion
        # if not np.all(D == 0):
        undistorted = cv2.fisheye.undistortPoints(this_corners, K, D, P=K)
        # check whether undistorted and this_corners are almost the same
