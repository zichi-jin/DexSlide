...
def main(session_dir):
    for session in session_dir:
        ...
        print('input_dir first:', input_dir)
        with ExifToolHelper() as et:
            for mp4_path in input_mp4_paths:
                ...
                # start_date = mp4_get_start_datetime(str(mp4_path))
                start_date = os.path.getmtime(str(mp4_path))
                ...
                # TODO - make it fit to sub directories!!! 
                if out_dname == 'mapping':
                    # find the json file with mapping in name, like *mapping*.json
                    json_files = list(input_dir.parent.glob('*mapping*.json'))
                    if len(json_files) == 0:
                        print(f"No mapping json file found in {input_dir.parent}, using the largest json.")
                        # find the largest json file
                        json_files = list(input_dir.parent.glob('*.json'))
                        max_size = -1
                        max_path = None
                        for json_path in json_files:
                            ...
                            if size > max_size:
                                max_size = size
                                max_path = json_path
                        print(f"Using largest json file {max_path.name} for video {input_dir}")
                        json_path = max_path
                        
                    else:
                        if len(json_files) > 1:
                            print(f"Multiple mapping json files found in {this_out_dir}, using the first one.")
                        json_path = json_files[0]
                        print(f"Found mapping json file {json_path.name} for video {this_out_dir}")
                    # move it to ./mapping/imu_data.json
                    out_json_path = this_out_dir.joinpath('imu_data.json')
                    shutil.move(json_path, out_json_path)
                else:
                    # move corresponding imu file if exists and rename
                    m = re.match(r"rgb_video_(.+)\.mp4", mp4_path.name, re.IGNORECASE)
                    if m:
                        timestamp = m.group(1)
                        imu_fname = f"imu_data_{timestamp}.json"
                        imu_path = mp4_path.parent.parent.joinpath(imu_fname)
                        print(f"Found IMU file {imu_path} for video {this_out_dir}")
                        if imu_path.exists():
                            out_imu_path = this_out_dir.joinpath('imu_data.json')
                            shutil.move(imu_path, out_imu_path)
                            print(f"Moved and renamed IMU file {imu_fname} to {out_imu_path}")
