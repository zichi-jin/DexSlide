...
@click.option('-nm', '--no_mask', is_flag=True, default=False, help="Run without slam_mask.png")
def main(input_dir, map_path, docker_image, num_workers, max_lost_frames, timeout_multiple, no_docker_pull, no_mask):
    ...
    input_video_dirs = [x.parent for x in input_dir.glob('data*/raw_video.mp4')] # data
    input_video_dirs += [x.parent for x in input_dir.glob('aurco/raw_video.mp4')]
    # input_video_dirs += [x.parent for x in input_dir.glob('mapping/raw_video.mp4')]
    print(f'Found {len(input_video_dirs)} video dirs')
    # print(input_video_dirs)
    all_have_traj = all(x.joinpath('camera_trajectory.csv').is_file() for x in input_video_dirs)
    if all_have_traj:
        print("All video dirs already have camera_trajectory.csv, skipping SLAM.")
        return
    # if map_path is None:
    ...
    mask_mount_source = map_path.parent.joinpath('slam_mask.png')
    if (not no_mask) and (not mask_mount_source.is_file()):
        print(f"⚠️ slam_mask.png not found in {map_path.parent}; running SLAM without mask.")
    ...
    with tqdm(total=len(input_video_dirs)) as pbar:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for video_dir in tqdm(input_video_dirs):
                ...
                mask_path = pathlib.Path('/map').joinpath('slam_mask.png')
                ...
                should_use_mask = (not no_mask) and mask_mount_source.is_file()
                if should_use_mask:
                    cmd.extend([
