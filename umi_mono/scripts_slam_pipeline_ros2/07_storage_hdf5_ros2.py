...
def process_bag_dir(
    ...
    if not trajectory_path.exists():
        print(f"❌ {trajectory_path} does not exist. Skipping {bag_name}.")
        return

    df = pd.read_csv(trajectory_path)
    if df["is_lost"].all():
        print(f"❌ all frames are lost in {trajectory_path}, skipping {bag_name}.")
        return

    first_valid_idx = int(df.index[df["is_lost"] == False][0])
    end_valid_idx = int(df.index[df["is_lost"] == False][-1])
    ...
    lost_count = int(df["is_lost"].sum())
    if lost_count > total_lost:
        print(f"❌ Too many lost frames ({lost_count}). Skipping {bag_name}.")
        return

    max_consecutive_lost = (df["is_lost"] != df["is_lost"].shift()).cumsum()
    lost_streaks = df.groupby(max_consecutive_lost)["is_lost"].sum()
    max_lost_streak = int(lost_streaks.max()) if not lost_streaks.empty else 0
    if max_lost_streak > lost_threshold:
        print(
            f"❌ Too many consecutive lost frames ({max_lost_streak}). Skipping {bag_name}."
        )
        return
    ...
    sensor = []
    sensor_times = []
    topics = [image_topic, sensor_topic]
    for topic, ts, msg in iter_deserialized_messages(bag_dir, topics=topics):
        if topic == image_topic:
            wrist_imgs.append(image_msg_to_array(msg))
            wrist_times.append(ts)
        elif topic == sensor_topic:
            try:
                sensor.append(extract_scalar_from_msg(msg))
                sensor_times.append(ts)
            except ValueError:
                continue

    wrist_imgs = np.stack(wrist_imgs) if wrist_imgs else np.empty((0,))
    sensor = np.array(sensor) if sensor else np.empty((0,))
    ...
    print(
        f"Found {len(wrist_imgs)} wrist images and {len(sensor)} sensor data points in {bag_name}."
    )

    if len(wrist_imgs) == 0:
        print(f"❌ No wrist images in {bag_name}, skipping.")
        return

    max_frame_idx = len(wrist_imgs) - 1
    if first_valid_idx > max_frame_idx:
        print(
            f"❌ first valid SLAM frame {first_valid_idx} exceeds image count {len(wrist_imgs)}. Skipping {bag_name}."
        )
        return

    end_valid_idx = min(end_valid_idx, max_frame_idx)
    crop_len = end_valid_idx - first_valid_idx + 1
    if crop_len <= 0:
        print(f"❌ Invalid crop length for {bag_name}, skipping.")
        return

    df = df.iloc[:crop_len].reset_index(drop=True)
    tx_slam_traj = tx_slam_traj[:crop_len]
    qpos = qpos[:crop_len]
    action = action[:crop_len]

    if sensor_times.size == 0 or sensor.size == 0:
        print(f"⚠️ No {sensor_topic} in {bag_name}; filling sensor values with zeros.")
    ...
    if len(cropped_sensor) != crop_len:
        cropped_sensor = np.resize(cropped_sensor, (crop_len,))
    ...
    if tag_detection_path.is_file():
        ...
        print(
            f"⚠️ {tag_detection_path} not found. ArUco-{aruco_tag_id} trajectory will be invalid."
        )
    ...
    if n_valid < aruco_min_valid_frames:
        print(
            ...
            f"Need >= {aruco_min_valid_frames}. Skipping {bag_name}."
        )
        return
    ...
    tx_aruco10_world = interpolate_fused_world_trajectory(
        tx_world_raw=tx_aruco10_world_raw,
    ...
    aruco10_pose_world[:, 3:] = (
        R.from_matrix(tx_aruco10_world[:, :3, :3]).as_quat().astype(np.float32)
    )
    ...
    with h5py.File(out_path, "w") as f:
        ...
        aruco.create_dataset(
            "tx_aruco13_slamworld", data=tx_aruco13_slamworld.astype(np.float32)
        )
        ...
        aruco.create_dataset(
            "tx_aruco10_world_raw", data=tx_aruco10_world_raw.astype(np.float32)
        )
        aruco.create_dataset(
            "tx_aruco10_world", data=tx_aruco10_world.astype(np.float32)
        )
...
@click.option(
    ...
    required=False,
    default=None,
    help="Path to 4x4 tx_eye_hand matrix (optional). If not provided, identity is used.",
)
@click.option(
    "--tx_slam_tag",
    required=False,
    default=None,
    help="Path to tx_slam_tag.json for ArUco-13 world calibration.",
)
@click.option(
    "--aruco_tag_id",
    default=10,
    type=int,
    help="ArUco marker id to track in wrist camera detections.",
)
@click.option(
    "--tag_detection_name",
    default="tag_detection_wrist.pkl",
    help="Detection filename under each demos/<bag_name> folder.",
)
@click.option(
    "--aruco_min_valid_frames",
    default=30,
    type=int,
    help="Minimum valid fused ArUco frames required.",
)
@click.option(
    "--aruco_max_missing_ratio",
    default=0.75,
    type=float,
    help="Maximum allowed missing ratio in fused ArUco trajectory.",
)
@click.option(
    "--aruco_max_consecutive_missing",
    default=10,
    type=int,
    help="Maximum allowed consecutive missing fused ArUco frames.",
)
@click.option(
    "--total_lost",
    default=200,
    type=int,
    help="Total number of lost frames allowed in camera_trajectory.csv",
)
@click.option(
    "--lost_threshold",
    default=50,
    type=int,
    help="Threshold for constant lost frames in camera_trajectory.csv",
)
@click.option(
    "--image_topic",
    default="/camera/camera/color/image_raw",
    show_default=True,
    help="Wrist camera image topic in ROS2 bag.",
)
@click.option(
    "--sensor_topic",
    default="/sensor_data",
    show_default=True,
    help="Optional scalar sensor topic (e.g. gripper width).",
)
@click.option(
    "--max_workers", default=8, type=int, help="Maximum number of worker threads"
)
def main(
    input,
    total_lost,
    ...
    path = pathlib.Path(input).expanduser().absolute()

    bag_dirs = [
        d for d in discover_ros2_bag_dirs(path) if d.name not in ("aurco", "mapping")
    ]
    if not bag_dirs:
        print("No ROS2 bag directories found in the specified directory.")
        return
    ...
    if tx_eye_hand is None:
        ...
        eye_hand_path = pathlib.Path(tx_eye_hand).expanduser().absolute()
        ...
        if tx_cam_tcp.shape != (4, 4):
            raise ValueError(
                f"Expected 4x4 matrix in {eye_hand_path}, got shape {tx_cam_tcp.shape}"
            )
    ...
    if tx_slam_tag is None:
        ...
        slam_tag_path = pathlib.Path(tx_slam_tag).expanduser().absolute()
        ...
        if tx_slam_aruco13.shape != (4, 4):
            raise ValueError(
                f"Expected 4x4 tx_slam_tag in {slam_tag_path}, got shape {tx_slam_aruco13.shape}"
            )
        tx_aruco13_slamworld = np.linalg.inv(tx_slam_aruco13)

    print(
        f"Found {len(bag_dirs)} ROS2 bag directories. Processing with {max_workers} threads..."
    )
