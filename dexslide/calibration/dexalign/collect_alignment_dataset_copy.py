...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-kept-frames", type=int, default=120)
...
def main() -> int:
    ...
    try:
        ...
        print("[dexalign-collect] capture=PAUSED，聚焦 3D 窗口后按 SPACE 切换。")
        ...
        while plt.fignum_exists(fig.number) and not preview.stop_event.is_set():
            ...
            info_text.set_text(
                "\n".join(
                    [
                        ...
                        f"depth_invalid = {int(np.count_nonzero(~np.isfinite(depth_mm)))}",
                    ]
                )
            )
            ...
            camera_overlay = _compose_camera_overlay(
                ...
                extra_status_lines=[
                    f"capture={'ON' if capture_enabled else 'PAUSED'} key=SPACE",
                    f"capture_kind={str(args.capture_kind).lower()}",
                    f"collect kept={stats.frames_kept}/{int(args.target_kept_frames)} seen={stats.total_frames_seen}",
                    f"decision={decision_text}",
                ],
            )
