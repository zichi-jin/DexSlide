...
from dexslide.calibration.dexalign import objective as dexalign_objective
from dexslide.calibration.dexalign import pipeline_v2 as dexalign_pipeline
from dexslide.calibration.dexalign import weights as dexalign_weights
...
def _override_all_loss_weights(uniform_weight: float = 1.0) -> None:
    value = float(uniform_weight)
    if not np.isfinite(value) or value < 0.0:
        raise ValueError(f"uniform_weight must be a finite non-negative scalar, got {uniform_weight!r}")

    for name in dexalign_weights.KEYPOINT_CLASS_WEIGHT_MAP:
        dexalign_weights.KEYPOINT_CLASS_WEIGHT_MAP[name] = value
    dexalign_objective.KEYPOINT_CLASS_WEIGHTS[...] = value
    dexalign_pipeline.KEYPOINT_CLASS_WEIGHTS[...] = value

    dexalign_weights.STEP2_THUMB_SCALE_REG_WEIGHT = value
    dexalign_weights.STEP2_THUMB_BIAS_REG_WEIGHT = value
    dexalign_weights.STEP2_FOUR_FINGERS_SCALE_REG_WEIGHT = value
    dexalign_weights.STEP2_FOUR_FINGERS_BIAS_REG_WEIGHT = value
    dexalign_weights.STEP2_SCALE_REG_WEIGHTS[...] = value
    dexalign_weights.STEP2_BIAS_REG_WEIGHTS[...] = value
    dexalign_pipeline.STEP2_SCALE_REG_WEIGHTS[...] = value
    dexalign_pipeline.STEP2_BIAS_REG_WEIGHTS[...] = value

    dexalign_weights.STEP3_TRANSLATION_SAMPLE_WEIGHT = value
    dexalign_weights.STEP3_TRANSLATION_PRIOR_WEIGHT = value
    dexalign_pipeline.STEP3_TRANSLATION_SAMPLE_WEIGHT = value
    dexalign_pipeline.STEP3_TRANSLATION_PRIOR_WEIGHT = value
...
def _resolve_dataset_pair(
    *,
    ...
    if loaded_s2.num_frames <= 0:
        raise ValueError(f"dataset_s2 is empty: {resolved_s2_path}")
    ...
    if dataset_s1 is not None:
        ...
        if loaded_s1.num_frames <= 0:
            raise ValueError(
                "dataset_s1 exists but contains 0 frames. Remove the broken S1 file or recapture S1; "
                "DexAlign will not silently drop it and fall back to S2. "
                f"path={resolved_s1_path}"
            )
    return (
        loaded_s1,
...
def run_alignment_optimization(
    *,
    ...
    resolved_log_fn = log_fn if log_fn is not None else print
    ...
    if show_progress:
        resolved_log_fn(
            "[dexalign2] optimize: start "
            f"s1_frames={0 if dataset_s1 is None else dataset_s1.num_frames} "
            f"s2_frames={dataset_s2.num_frames}"
        )
    ...
    if write_outputs and output_path is not None:
        ...
        joint_summary = dict(summary_metrics["step2"])
        joint_summary.pop("joint_scale", None)
        joint_summary.pop("joint_bias_rad", None)
    ...
    if show_progress:
        resolved_log_fn(
            "[dexalign2] optimize: done "
            f"used_frames={int(np.count_nonzero(run_result.step3.final_eval.used_frame_mask))} "
            f"final_mean_error_mm={float(np.nanmean(run_result.step3.final_eval.frame_mean_error_mm)):.3f} "
            f"output_dir={str(output_path) if output_path is not None else '<none>'}"
        )
    ...
    if run_result.step3.optimize_thumb_base_rx:
        x_opt_parts.append(np.array([run_result.step3.thumb_base_rx_delta_rad], dtype=np.float64))
    x_opt = np.concatenate(x_opt_parts, axis=0)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow Step 3 to rotate thumb_base and the downstream thumb chain around the hand-frame x axis.",
    )
...
def main() -> int:
    ...
    # 临时实验开关：取消注释后，把 keypoint / step2 / step3 的权重系数全部覆写成同一个值。
    # _override_all_loss_weights(uniform_weight=1.0)
    ...
    result = run_alignment_optimization(
        ...
        optimize_thumb_base_rx=bool(args.optimize_thumb_base_rx),
        show_progress=True,
    )
