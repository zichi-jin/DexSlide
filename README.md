...
dexslide 是一款开源外骨骼数据手套，用于采集人手 20 个关节角度并实时重建手部姿态。当前仓库包含下位机 STM32 固件、上位机 Python 标定与可视化工具、硬件资料和实验脚本。
...
组装 dexslide 机械结构、编码器、ADS1115 ADC 板和 STM32F103 控制板。当前主线下位机工程位于 `firmware/dexslide_stm32/`，负责读取 5 个 ADS1115 上的 20 路编码器数据，并通过 USB CDC 串口输出。
...
```text
software/python/assets/skeletons/skeleton.json
software/python/assets/skeletons/offline_bone_mm_results.json
```
...
拍完骨骼后，还需要告诉上位机“串口 ADC 数值如何换算成角度”。这个过程由 firmware 子工程里的脚本完成。

1. 逐关节标定：

```bash
cd firmware/dexslide_stm32/scripts
python glove_calibrate.py --port /dev/ttyACM0
```

脚本会按 20 个关节顺序引导你采集 0 度和参考角度。大多数关节使用 90 度参考；部分 `MCP_back` 可以输入自定义参考角，或在机械动作不方便时选择默认比例。标定结果默认保存为：

```text
firmware/dexslide_stm32/scripts/glove_calibration.json
```

2. 检查原始串口数据：

```bash
python ads_live_monitor.py --port /dev/ttyACM0
```

3. 检查映射后的角度是否合理：

```bash
python ads_live_monitor.py --port /dev/ttyACM0 --angles --calib-file glove_calibration.json
```
...
`calibrate-zero` 已从 `main.py` 移除。当前角度零位和比例关系由 `firmware/dexslide_stm32/scripts/glove_calibrate.py` 统一完成。
...
└── software/
    └── python/
        ...
        │   │   ├── offline_a4_bone_mm.py
        │   │   └── plot_compare_skeletons.py
        │   ├── kinematics/
        │   │   ├── hand_model.py
        │   │   └── live_hand.py
        │   ├── visualization/
        │   │   ├── demo_20dof_matplotlib.py
        │   │   ├── live_matplotlib.py
        │   │   └── skeleton_plot.py
        │   ├── paths.py
        │   ├── serial_angles.py
...
- 触觉：在 `firmware/` 中整理和烧写触觉采样代码，配合 System B 硬件。
